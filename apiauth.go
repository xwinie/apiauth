...
func APISecretAuth(f AppIDToAppSecret, timeout int) beego.FilterFunc {
	return func(ctx *context.Context) {
		...
		q:=ctx.Input.Query("signature")
		s:=Signature(appsecret, ctx.Input.Method(), ctx.Request.Form, ctx.Input.URL())
		fmt.Printf("11111111:" +q+"\n")
		fmt.Printf("11111111:"+ s+"\n")
		if q!= s {
		//Signature(appsecret, ctx.Input.Method(), ctx.Request.Form, ctx.Request.RequestURI) {
			ctx.ResponseWriter.WriteHeader(403)
			//fmt.Printf("33333:"+"\n")
...
func Signature(appsecret, method string, params url.Values, RequestURI string) (result string) {
	...
	//	pa[k] = v[0]
	//}
	//fmt.Printf("11111111:"+"  "+query.Encode()+"\n")
	delete(params, "signature");
	//vs := mapSorter(params)
	//vs.Sort()
	//for i := 0; i < vs.Len(); i++ {
	...
	//fmt.Printf("11111111:"+"  "+string(vs.Keys)+string(vs.Vals)+"\n")
	//fmt.Printf("22:"+"  "+params.Encode()+"\n")
	stringToSign := fmt.Sprintf("%v%v%v\n", method,params, RequestURI)
	//stringToSign := fmt.Sprintf("%v\n%v\n", method, query)
	sha256 := sha256.New
	hash := hmac.New(sha256, []byte(appsecret))
	hash.Write([]byte(stringToSign))
	//fmt.Printf("stringToSign:"+stringToSign+"  ")
	//fmt.Printf("111:"+ base64.StdEncoding.EncodeToString(hash.Sum(nil))+"\n")
